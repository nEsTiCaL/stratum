...
ALL = ("phi-4-mini", "qwen2.5-coder", "qwen3-8b", "r1-distill", "qwen3-8b-q8")
...
def _policy(**over) -> CapacityPolicy:
    base = {
        "budget_mb": 13000,
        "max_parallel": 2,
        "resident_set": ("phi-4-mini", "qwen2.5-coder"),
        "allowed_models": ALL,
        "reserve_mb": 1024,
        "gpu_id": 0,
    }
...
class TestModelConfig:
    def test_known_models_and_costs(self):
        assert MODEL_CONFIG["phi-4-mini"].vram_mb == 3000
...
class TestLoadPolicy:
    def test_reads_toml(self, tmp_path):
        ...
        p.write_text(
            ...
            "max_parallel = 2\n"
            'resident_set = ["phi-4-mini", "qwen2.5-coder"]\n'
            'allowed_models = ["phi-4-mini", "qwen2.5-coder"]\n'
            "reserve_mb = 1024\n"
            "gpu_id = 0\n",
            encoding="utf-8",
        )
        ...
        assert pol.resident_set == ("phi-4-mini", "qwen2.5-coder")
        assert pol.allowed_models == ("phi-4-mini", "qwen2.5-coder")
...
class TestResolveProfiles:
    ...
    def test_profile_c_server(self):
        pol = _policy(
            budget_mb=44000,
            max_parallel=4,
            resident_set=("phi-4-mini", "qwen2.5-coder", "qwen3-8b", "r1-distill"),
            allowed_models=ALL,
        )
...
class TestAutoDetect:
    ...
    def test_cpu_default_no_policy(self):
        ...
        assert r.is_cpu is True
        assert r.policy.resident_set == ("phi-4-mini",)
        assert r.policy.allowed_models == ("phi-4-mini",)
        assert r.max_parallel == 1
